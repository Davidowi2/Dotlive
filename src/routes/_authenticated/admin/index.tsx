/**
 * /admin — Admin dashboard home. (MINIMAL DEBUG VERSION)
 */

import { createFileRoute, Link } from "@tanstack/react-router";
import { useQuery } from "@tanstack/react-query";
import { Activity, Users, Shield, Coins, AlertTriangle, ArrowRight, TrendingUp, Clock, Wallet as WalletIcon, FileText, CheckCircle2 } from "lucide-react";
...
interface AdminStats {
  users: { total: number; today: number; week: number; admins: number; banned: number };
  ventures: { total: number };
  communities: { total: number };
  challenges: { total: number };
  tokenOps: { total: number; recent: Array<{ id: string; type: string; amount: number | string; actor: string; reason: string | null; created_at: string }> };
  wallets: { total: number; totalDot: number | string };
  timestamp: string;
}
...
function AdminDashboardPage() {
      ...
      <>
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Loading platform stats…</p>
          </div>
        </div>
    ...
    return (
      <>
        <div className="flex items-center justify-between">
          <div>
            <h1 className="font-display text-3xl">Dashboard</h1>
            <p className="mt-1 text-sm text-muted-foreground">Could not load stats.</p>
          </div>
        </div>
  ...
  return (
    <>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stat("Users total", stats.users.total.toLocaleString(), `${stats.users.today} new today · ${stats.users.week} this week`, Users)}
        {stat("Admins", stats.users.admins, "admin + super_admin", Shield)}
        {stat("Ventures", stats.ventures.total, "all statuses", FileText)}
        {stat("DOT in circulation", Number(stats.wallets.totalDot).toLocaleString(), `${stats.wallets.total} wallets`, Coins)}
