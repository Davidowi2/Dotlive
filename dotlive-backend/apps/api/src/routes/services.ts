...
export async function marketplaceRoutes(app: FastifyInstance) {
  ...
  /** POST /api/orders — client creates a gig order; funds moved to escrow */
  app.post("/orders", { preHandler: app.authenticate }, async (req, reply) => {
    ...
    // Immediately debit the buyer and reserve funds in lockedBalance.
    try {
      ...
      return reply.code(402).send({ error: e?.message ?? "Insufficient balance to place order" });
    }
    ...
    return reply.send({
      ...
      notice: "DOT is reserved in escrow and will be released when the order completes.",
    });
  });
  ...
  app.patch<{ Params: { id: string } }>("/orders/:id/deliver", { preHandler: app.authenticate }, async (req, reply) => {
    ...
    const updated = await db
      ...
      .set({ status: "delivered", deliveryNote: parsed.data.deliveryNote, deliveredAt: new Date(), updatedAt: new Date() } as any)
      .where(eq(serviceOrders.id, req.params.id))
      .returning();
    return reply.send({ order: updated[0] });
  });

  /** PATCH /api/orders/:id/complete — client accepts, releases escrow */
  app.patch<{ Params: { id: string } }>("/orders/:id/complete", { preHandler: app.authenticate }, async (req, reply) => {
    ...
    // Move lockedBalance from buyer to builder, update earnedLifetime.
    try {
      await db.transaction(async (tx) => {
        await tx
          .update(wallets)
